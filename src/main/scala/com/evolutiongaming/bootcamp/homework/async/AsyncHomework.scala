package com.evolutiongaming.bootcamp.homework.async
...
object AsyncHomework extends App {
    ...
    val url: String = args match {
        case Array(url) => url
        case Array() => throw new Exception("Mandatory parameter missing URL!")
        case _ => throw new Exception("Should be one parameter URL!")
    }

    fetchPageBody(url = url)
      .flatMap(findLinkUrls)
      .flatMap(Future.traverse(_)(fetchServerName))
      .map(_.distinct.sorted.foreach(_.foreach(println)))
